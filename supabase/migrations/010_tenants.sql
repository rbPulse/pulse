...
--     - function auth.current_tenant_ids()  (helper used by later RLS policies)
...
-- ── Helper: auth.current_tenant_ids() ──────────────────────────────────────
-- Returns the set of tenant UUIDs the calling user has access to, via either
-- a staff membership or a patient enrollment. Later RLS policies on
-- tenant-scoped tables call this function instead of re-writing the subquery
-- 17 times. Stable so Postgres can cache within a query.

CREATE OR REPLACE FUNCTION auth.current_tenant_ids()
RETURNS SETOF uuid
...
-- Platform-role check — used in companion policies that let Pulse internal
-- users see across tenants.
...
CREATE POLICY "tenants_read" ON tenants
  ...
  USING (
    id IN (SELECT auth.current_tenant_ids())
...
CREATE POLICY "memberships_self_read" ON tenant_memberships
  ...
  USING (
    user_id = auth.uid()
    OR tenant_id IN (SELECT auth.current_tenant_ids())
    OR auth.is_platform_user()
  );
...
-- DROP POLICY IF EXISTS "tenants_platform_write"       ON tenants;
-- DROP POLICY IF EXISTS "tenants_read"                 ON tenants;
-- DROP FUNCTION IF EXISTS auth.is_platform_user();
-- DROP FUNCTION IF EXISTS auth.current_tenant_ids();
